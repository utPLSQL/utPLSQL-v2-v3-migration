CREATE OR REPLACE PACKAGE BODY utassert2 IS
...
/*
  FUNCTION id(name_in IN varchar2) RETURN number IS
    retval number;
  BEGIN
  ...
  FUNCTION NAME(id_in IN number) RETURN varchar2 IS
    retval varchar2;
  BEGIN
...
*/
  ...
  (
   ...
   ,register_in   IN BOOLEAN := TRUE
    ...
    l_failure BOOLEAN := NOT check_this_in OR (check_this_in IS NULL AND NOT null_ok_in);
  BEGIN
    /* START chrisrimmer 42694 */
  ...
  BEGIN
    IF l_failure
    THEN
      this(outcome_in, failure_msg_in, check_this_in, null_ok_in, raise_exc_in, register_in);
    ELSE
  ...
  BEGIN
    RETURN(type_in || ' "' || msg_in || '" Expected "' || against_this_in || '" and got "' ||
           check_this_in || '"');
  END;
  ...
  BEGIN
    ...
    eval_block       := 'DECLARE
        ...
        BEGIN
           b_result := ' || using_in || ';
           IF b_result THEN :result := ''' || utplsql.c_yes || '''; ' ||
                        'ELSIF NOT b_result THEN :result := ''' || utplsql.c_no || '''; ' ||
                        'ELSE :result := NULL;
           END IF;
        END;';
    ...
    this(outcome_in     => outcome_in
        ...
        ,failure_msg_in => eval_description || ' evaluated to FALSE'
        ,check_this_in  => utplsql.vc2bool(eval_result)
        ,null_ok_in     => null_ok_in
        ,raise_exc_in   => raise_exc_in);
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      this(outcome_in     => outcome_in
          ,success_msg_in => NULL
          ,failure_msg_in => 'Error in ' || eval_description || ' SQLERRM: ' || SQLERRM
          ,check_this_in  => FALSE
          ,null_ok_in     => null_ok_in
          ,raise_exc_in   => raise_exc_in);
  END;
  ...
  BEGIN
    ...
    eval(l_id, msg_in, using_in, value_name_in, null_ok_in, raise_exc_in);
  END;
  ...
  */
  ...
  BEGIN
    ...
    this(outcome_in    => outcome_in
        ...
        ,check_this_in => (nvl(check_this_in = against_this_in, FALSE)) OR
                          (check_this_in IS NULL AND against_this_in IS NULL AND null_ok_in)
        ,null_ok_in    => FALSE
        ,raise_exc_in  => raise_exc_in);
  END;

  FUNCTION b2v(bool_in IN BOOLEAN) RETURN VARCHAR2 IS
  BEGIN
    IF bool_in
    THEN
      RETURN 'TRUE';
  ...
  BEGIN
    ...
    this(outcome_in
        ...
        ,(check_this_in = against_this_in) OR
         (check_this_in IS NULL AND against_this_in IS NULL AND null_ok_in)
        ,FALSE
        ,raise_exc_in);
  END;
  ...
  BEGIN
    ...
    this(outcome_in
        ,message_expected('EQ'
                         ,msg_in
                         ,to_char(check_this_in, 'MON-DD-YYYY HH:MI:SS')
                         ,to_char(against_this_in, 'MON-DD-YYYY HH:MI:SS'))
        ,(check_this_in = against_this_in) OR
         (check_this_in IS NULL AND against_this_in IS NULL AND null_ok_in)
        ,FALSE
        ,raise_exc_in);
  ...
  BEGIN
    ieqminus(outcome_in
            ,message('EQTABLE'
                    ,msg_in
                    ,'Contents of "' || check_this_in ||
                     utplsql.ifelse(check_where_in IS NULL, '"', '" WHERE ' || check_where_in) ||
                     ' does ' || c_not_placeholder -- utplsql.ifelse (NOT l_failure, NULL, ' not ')
                     || 'match "' || against_this_in ||
                     utplsql.ifelse(against_where_in IS NULL, '"', '" WHERE ' || against_where_in))
            ,'SELECT T1.*, COUNT(*) FROM ' || check_this_in || ' T1  WHERE ' ||
             nvl(check_where_in, '1=1') || ' GROUP BY ' || collist(check_this_in)
            ,'SELECT T2.*, COUNT(*) FROM ' || against_this_in || ' T2  WHERE ' ||
             nvl(against_where_in, '1=1') || ' GROUP BY ' || collist(against_this_in)
            ,'Table Equality'
            ,raise_exc_in);
  END;
  ...
  BEGIN
    ieqminus(outcome_in
            ,message('EQTABCOUNT'
                    ,msg_in
                    ,'Row count of "' || check_this_in ||
                     utplsql.ifelse(check_where_in IS NULL, '"', '" WHERE ' || check_where_in) ||
                     ' does ' || c_not_placeholder -- utplsql.ifelse (NOT l_failure, NULL, ' not ')
                     || 'match that of "' || against_this_in ||
                     utplsql.ifelse(against_where_in IS NULL, '"', '" WHERE ' || against_where_in))
            ,'SELECT COUNT(*) FROM ' || check_this_in || '  WHERE ' || nvl(check_where_in, '1=1')
            ,'SELECT COUNT(*) FROM ' || against_this_in || '  WHERE ' || nvl(against_where_in, '1=1')
  ...
  BEGIN
    ieqminus(outcome_in
            ,message('EQQUERY'
                    ,msg_in
                    ,'Result set for "' || check_this_in || ' does ' || c_not_placeholder -- utplsql.ifelse (NOT l_failure, NULL, ' not ')
                     || 'match that of "' || against_this_in || '"')
            ,check_this_in
            ,against_this_in
            ,'Query Equality'
  ...
  BEGIN
    ...
    this(outcome_in
        ,message('EQQUERYVALUE'
                ,msg_in
                ,'Query "' || check_query_in || '" returned value "' || l_value || '" that does ' ||
                 utplsql.ifelse(l_success, NULL, ' not ') || 'match "' || against_value_in || '"')
        ,l_success
        ,FALSE
        ,raise_exc_in);
  ...
  BEGIN
    ...
    this(outcome_in
        ,message('EQQUERYVALUE'
                ,msg_in
                ,'Query "' || check_query_in || '" returned value "' ||
                 to_char(l_value, 'DD-MON-YYYY HH24:MI:SS') || '" that does ' ||
                 utplsql.ifelse(l_success, NULL, ' not ') || 'match "' ||
   ...
   ,raise_exc_in        IN BOOLEAN := FALSE
    ...
    BEGIN
      ...
      this(outcome_in
          ,message('EQFILE'
                  ,msg_in
                  ,utplsql.ifelse(line_set_in, ' Line ' || linenum_in || ' of ', NULL) || 'File "' ||
                   file_descrip(check_this_in, check_this_dir_in) || '" does ' ||
                   utplsql.ifelse(val, NULL, ' not ') || 'match "' ||
  ...
  EXCEPTION
    WHEN cant_open_check_file THEN
      cleanup(FALSE, msg_in => 'Unable to open ' || file_descrip(check_this_in, check_this_dir_in));
  ...
  BEGIN
    ...
    this(outcome_in
        ,message('EQPIPE'
                ,msg_in
                ,utplsql.ifelse(msgset, ' Message ' || msgnum || ' of ', NULL) || 'Pipe "' ||
                 check_this_in || '" does ' || utplsql.ifelse(same_message, NULL, ' not ') ||
                 'match "' || against_this_in || '".')
        ,same_message
        ,FALSE
        ,raise_exc_in
  ...
  BEGIN
    ...
    THEN
      eqcheck := '(' || v_check || '(cindx) = ' || v_against || ' (aindx)) OR ' || '(' || v_check ||
                 '(cindx) IS NULL AND ' || v_against || ' (aindx) IS NULL)';
    ELSE
    ...
    RETURN('DECLARE
             ...
             aend PLS_INTEGER := NVL (:aendit, ' || against_this_in || '.' ||
           lastrowproc_in || ');
  ...
  BEGIN
    ...
    RETURN message(assert_name
                  ...
                  ,utplsql.ifelse(success_in
                                 ,NULL
                                 ,' Row ' || nvl(to_char(agrow_in), '*UNDEFINED*') || ' of ') ||
                   'Collection "' || agcoll_in || '" does ' ||
                   utplsql.ifelse(success_in, NULL, ' not ') || 'match ' ||
                   utplsql.ifelse(success_in
                                 ,NULL
                                 ,' Row ' || nvl(to_char(chkrow_in), '*UNDEFINED*') || ' of ') ||
                   chkcoll_in || '".');
  END;
  ...
  BEGIN
    this(outcome_in
        ,message_expected('ISNOTNULL', msg_in, check_this_in, 'NOT NULL')
  ...
  BEGIN
    this(outcome_in
        ,message_expected('ISNULL', msg_in, check_this_in, '')
  ...
  BEGIN
    this(outcome_in
        ,message_expected('ISNOTNULL', msg_in, utplsql.bool2vc(check_this_in), 'NOT NULL')
        ,check_this_in IS NOT NULL
        ,FALSE
        ,raise_exc_in);
  END;
  ...
  BEGIN
    this(outcome_in
        ,message_expected('ISNULL', msg_in, utplsql.bool2vc(check_this_in), '')
        ,check_this_in IS NULL
        ,TRUE
        ,raise_exc_in);
  END;
  ...
  (
   ...
   ,against_exc_in IN VARCHAR2
    ...
    v_block            VARCHAR2(32767) := 'BEGIN ' || rtrim(rtrim(check_call_in), ';') || ';
               :indicator := 0;
             EXCEPTION
                WHEN ' || against_exc_in ||
                                          ' THEN
  ...
  BEGIN
    ...
    this(outcome_in
        ,message('RAISES'
                ,msg_in
                ,'Block "' || check_call_in || '"' ||
                 utplsql.ifelse(NOT (nvl(l_indicator = expected_indicator, FALSE))
                               ,' does not raise'
                               ,' raises ') || ' Exception "' || against_exc_in ||
                 utplsql.ifelse(l_indicator = expected_indicator
                               ,NULL
  ...
  (
   ...
   ,against_exc_in IN NUMBER
    ...
    v_block            VARCHAR2(32767) := 'BEGIN ' || rtrim(rtrim(check_call_in), ';') || ';
             ...
             EXCEPTION
                WHEN OTHERS
                   THEN IF SQLCODE = ' || against_exc_in ||
  ...
  BEGIN
    get_id(outcome_in, l_id);
    this(l_id, msg_in, check_this_in, null_ok_in, raise_exc_in, register_in);
  END;
  ...
  BEGIN
    get_id(outcome_in, l_id);
    eq(l_id, msg_in, check_this_in, against_this_in, null_ok_in, raise_exc_in);
  END;
  ...
  BEGIN
    get_id(outcome_in, l_id);
    eq(l_id, msg_in, check_this_in, against_this_in, null_ok_in, raise_exc_in, truncate_in);
  END;
  ...
  BEGIN
    get_id(outcome_in, l_id);
    eqqueryvalue(l_id, msg_in, check_query_in, against_value_in, raise_exc_in);
  END;
   ...
   ,raise_exc_in IN BOOLEAN := FALSE
    ...
    BEGIN
      ...
      this(outcome_in
          ,message('FILEEXISTS'
                  ,msg_in
                  ,'File "' || file_descrip(file_in, dir_in) || '" could ' ||
                   utplsql.ifelse(val, NULL, ' not ') || 'be opened for reading."')
          ,val
          ,FALSE
          ,raise_exc_in
          ,TRUE);
    END;
  BEGIN
    checkid := utl_file.fopen(dir_in, file_in, 'R' /* >= v9 */, max_linesize => 32767 /* >= v9 */);
    cleanup(TRUE, msg_in);
  EXCEPTION
    WHEN OTHERS THEN
  ...
  PROCEDURE showresults IS
  BEGIN
    g_showresults := TRUE;
  END;

  PROCEDURE noshowresults IS
  BEGIN
    g_showresults := FALSE;
  END;

  FUNCTION showing_results RETURN BOOLEAN IS
  BEGIN
  ...
  FUNCTION find_obj(check_this_in IN VARCHAR2) RETURN BOOLEAN IS
    v_st       VARCHAR2(20);
    v_err      VARCHAR2(100);
  ...
  /* START chrisrimmer 42694 */
  FUNCTION previous_passed RETURN BOOLEAN IS
  BEGIN
    RETURN g_previous_pass;
  END;

  FUNCTION previous_failed RETURN BOOLEAN IS
  BEGIN
  ...
  BEGIN
    ...
    THEN
      IF v_check_index IS NULL
         AND v_against_index IS NOT NULL
      THEN
        v_message := message('EQOUTPUT'
                            ,msg_in
                            ,'Extra line found at end of reference collection: ' ||
                             preview_line(against_this_in(v_against_index)));
      ELSIF v_check_index IS NOT NULL
            AND v_against_index IS NULL
      THEN
        v_message := message('EQOUTPUT'
                            ,msg_in
  ...
  BEGIN
    refc_table_name := utplsql_util.prepare_and_fetch_rc(proc_name
                                                        ,params
                                                        ,cursor_position
                                                        ,1
                                                        ,table_name);
